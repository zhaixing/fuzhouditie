...
- (id)initWithFrame:(CGRect)frame dataSource:(id)datasource delegate:(id) delegate {
    self = [super initWithFrame:frame];
    if (self) {
        self.backgroundColor = [UIColor clearColor];//设置成透明色
        currentExtendSection = -1;
        self.dropDownDataSource = datasource;// 设置文件资源
        self.dropDownDelegate = delegate;// 设置代理
        ...
        //初始化默认显示view
        CGFloat sectionWidth = (1.0*(frame.size.width)/sectionNum);// 计算宽度
        for (int i = 0; i < sectionNum; i++) {
            UIButton *sectionBtn = [[UIButton alloc] initWithFrame:CGRectMake(sectionWidth*i, 1, sectionWidth, frame.size.height-2)];// 初始化按钮
            sectionBtn.tag = SECTION_BTN_TAG_BEGIN + i;
            [sectionBtn addTarget:self action:@selector(sectionBtnTouch:) forControlEvents:UIControlEventTouchUpInside];// 点击事件
            ...
            [sectionBtn setTitle:sectionBtnTitle forState:UIControlStateNormal];// 设置按钮的文字
            [sectionBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
            sectionBtn.titleLabel.font = [UIFont boldSystemFontOfSize:15.0f];
...
-(void)showChooseListViewInSection:(NSInteger)section choosedIndex:(NSInteger)index
{
    ...
    [self.mTableView reloadData]; // 刷新下拉列表
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.numberOfLines = 0;// 一行显示不下，自动换行
    return cell;
}
